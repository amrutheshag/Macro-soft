...
public class InpatientWardsUnitsAddTestScript {
	WebDriver driver;
	FileInputStream fis;

	@Test
	public void InpatientWardsUnitsAddTestScriptTestScript() throws Exception {
		DOMConfigurator.configure("log4j.xml");
		System.out.println("\n*****************************************************\n");
		Log.info("\n*****************************************************\n");
		System.out.println("Executing Inpatient Wards/Units- Add");
		Log.info("Executing  Inpatient Wards/Units- Add ");
		...
		try {
			...
			for (int count = 1; count <= rowCount; count++) {
				...
				if (ExcelUtils.getCellData(count, 0).isEmpty()) {
					...
					System.out.println("\n----------------------------------------------------\n");
					System.out.println(
							"Inpatient Wards/Units Add- Running Iteration No:  " + ExcelUtils.getCellData(count, 0));
					System.out.println("\n----------------------------------------------------\n");
					Log.info("----------------------------------------------------");
					Log.info("Inpatient Wards/Units Add- Running Iteration No:  " + ExcelUtils.getCellData(count, 0));
					Log.info("----------------------------------------------------");
					addInpatientWardsUnitsMultiple(driver, inpatientwardsUnitsCode, inpatientwardsUnitsName);
				}
			}
		} catch (Exception e) {
	...
	public void addInpatientWardsUnitsMultiple(WebDriver driver, String InpatientWardsUnitsCode,
			String InpatientWardsUnitsName) throws Exception {
		...
		InpatientWardsUnitsObject.addInpatientWardsUnits(InpatientWardsUnitsCode, InpatientWardsUnitsName);
	...
	public void afterMethod() throws Exception {
		if (driver != null)
			driver.quit();
		System.out.println("Done with Inpatient Wards/Units-Add \n\n");
		Log.info("log4j -Done with Inpatient Wards/Units-Add \n\n");
		if (fis != null)
			fis.close();
	}
}