...
public class ProviderAddForMembers {

	static WebDriver driver;
	FileInputStream fis;
		@Test
		public void providerAddForMembersTestScript() throws Exception{
			DOMConfigurator.configure("log4j.xml");
			System.out.println("\n*****************************************************\n");
			Log.info("\n*****************************************************\n");
			System.out.println("Executing Providers- Add Providers for members");
			Log.info("Executing Providers- Add Providers for members");
			
			
			try{
				String Path=Constant.Path_TestData + Constant.File_TestDataToAdd_Providers_ForMembers;
		    	...
		    	for (int count=1; count<=rowCount; count++){
		    		...
		    		System.out.println("\n----------------------------------------------------\n");
		   		 	System.out.println("Provider Add for members- Running Iteration No:  " + ExcelUtils.getCellData(count,0));
		   		 	System.out.println("\n----------------------------------------------------\n");
		   		 	Log.info("----------------------------------------------------");
		   		 	Log.info("Provider Add for members- Running Iteration No:  " + ExcelUtils.getCellData(count,0));
		   		 	Log.info("----------------------------------------------------");
		   		 addProviderForMembersMultiple(driver,pLastname, pFirstname, pMiddleInitial, pCode, pNPI, pType, providerDegree, pDOB, pGender, pActiveFrom, pTerminationDate, pTAXid, pSpeciality,pSiteStartDate, pSiteEndDate,pSiteType, pTAXONOMYcode, pAddr, pCity, pState, pZIP, providerCounty,  pPhNo,providerFAX,    pUserField, specialitiesCurrentStatus, specialitiesEffectivePeriod, specialityInfo, IDType, IDNumber, IDEffectivePeriod, ProviderContractEndDate, payer, benefitClass, serviceSite, paymentsCreditToTIN, redirectToPayee, feeSchedule, vendorName, vendorType, vendorNPI, vendorAPID, vendorActiveStatusEndDate, vendorDefaultTaxID);}
			}
			}
		...
		System.out.println("Done with Providers-Add Providers for members \n\n");
		if (fis != null) 
				fis.close();
	}
