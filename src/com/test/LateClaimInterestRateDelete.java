...
public class LateClaimInterestRateDelete {
	WebDriver driver;
	FileInputStream fis;
		@Test
		public void LateClaimsInterestRatesDeleteTestScript() throws Exception{
			DOMConfigurator.configure("log4j.xml");
			System.out.println("\n*****************************************************\n");
			Log.info("\n*****************************************************\n");
			System.out.println("Executing Late Claims Interest Rates- delete");
			Log.info("Executing  Late Claims Interest Rates delete ");
			
			
			try{
				String Path=Constant.Path_TestData + Constant.File_LateClaimInterestRatesTestData_Delete;
		    	...
		    	for (int count=1; count<=rowCount; count++){
		    		...
		    		System.out.println("\n----------------------------------------------------\n");
		   		 	System.out.println("Late Claims Interest Rates delete- Running Iteration No:  " + ExcelUtils.getCellData(count,0));
		   		 	System.out.println("\n----------------------------------------------------\n");
		   		 	Log.info("----------------------------------------------------");
		   		 	Log.info("Late Claims Interest Rates delete- Running Iteration No:  " + ExcelUtils.getCellData(count,0));
		   		 	Log.info("----------------------------------------------------");
   		 	deleteInterestRatesMultiple(driver,effectiveDate, state, benefitProduct);
		    		}
		    	}
			...
			public void deleteInterestRatesMultiple(WebDriver driver,String effectiveDate, String state, String benefitProduct) throws Exception
			{
			...
			claimInterestObject.InterestRateDelete( effectiveDate, state, benefitProduct);
		...
		System.out.println("Done with Late Claim Interest Rates-delete  \n\n");
		if (fis != null)
				fis.close();
	}
}
