...
public class EOBCACloseoutTestScript {
	WebDriver driver;
	FileInputStream fis;
		@Test
		public void EOBCACloseoutExtractClaimsDataReportTestScript() throws Exception{
			DOMConfigurator.configure("log4j.xml");
			System.out.println("\n***************************************************************\n");
			Log.info("\n***************************************************************\n");
			System.out.println("Executing EOB CA Closeout extract claims data report");
			Log.info("Executing EOB CA Closeout extract claims data report");
			
			
			try{
				String Path=Constant.Path_TestData + Constant.File_Edit_EOBQueryData;
			   	int rowCount=ExcelUtils.setExcelFile(Path, "CA");
		    	for (int count=1; count<=rowCount; count++){
		    		...
		    		System.out.println("\n----------------------------------------------------\n");
		   		 	System.out.println("EOB CA Closeout Extract claims data report - Running Iteration No:  " + ExcelUtils.getCellData(count,0));
		   		 	System.out.println("\n----------------------------------------------------\n");
		   		 	Log.info("----------------------------------------------------");
		   		 	Log.info("EOB CA Closeout Extract claims data report- Running Iteration No:  " + ExcelUtils.getCellData(count,0));
		   		 	Log.info("----------------------------------------------------");
		...
		System.out.println("Done with EOB CA Closeout Extract claims data report \n\n");
		if (fis != null) 
				fis.close();
	}
