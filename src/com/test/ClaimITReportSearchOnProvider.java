...
public class ClaimITReportSearchOnProvider {
	WebDriver driver;
	FileInputStream fis;
		@Test
		public void ClaimITReportWithProviderTestScript() throws Exception{
			DOMConfigurator.configure("log4j.xml");
			System.out.println("\n*****************************************************\n");
			Log.info("\n*****************************************************\n");
			System.out.println("Executing Claim IT Report with Provider");
			Log.info("Executing Claim IT Report with Provider");
			...
			try{
		    	...
		    	for (int count=1; count<=rowCount; count++){
		    		...
		    		System.out.println("\n----------------------------------------------------\n");
		   		 	System.out.println("Claim IT Report with Provider- Running Iteration No:  " + ExcelUtils.getCellData(count,0));
		   		 	System.out.println("\n----------------------------------------------------\n");
		   		 	Log.info("----------------------------------------------------");
		   		 	Log.info("Claim IT Report with Provider- Running Iteration No:  " + ExcelUtils.getCellData(count,0));
		   		 	Log.info("----------------------------------------------------");
		   		 claimITReportWithProviderMultiple(driver, approvedServiceOnly, openClaimsOnly, extractClaimAndServiceData, denyServices, providerID, memberID, payor, from, to, claimQuery, insurerPayor, checkBilaterals, checkCCI, checkGlobalPeriods, checkLCPDiagnosis, assistantSurgeon,clientFault, providerIDInClaimITReport, payorInClaimITReport);
		    		}
		    	}
			...
			public void claimITReportWithProviderMultiple(WebDriver driver, String approvedServiceOnly, String openClaimsOnly, String extractClaimAndServiceData, String denyServices, String providerID, String memberID, String payor, String from, String to, String claimQuery, String insurerPayor, String checkBilaterals, String checkCCI, String checkGlobalPeriods, String checkLCPDiagnosis, String assistantSurgeon, String clientFault, String providerIDInClaimITReport, String payorInClaimITReport) throws Exception{
			...
			ClaimITReportPageObject.searchOnProviderPrintClaimITReport( approvedServiceOnly, openClaimsOnly, extractClaimAndServiceData, denyServices, providerID, memberID, payor, from, to, claimQuery, insurerPayor, checkBilaterals, checkCCI, checkGlobalPeriods, checkLCPDiagnosis, assistantSurgeon, clientFault, providerIDInClaimITReport, payorInClaimITReport);
			}

		@AfterMethod
		public void afterMethod() throws Exception {
		if (driver!=null) driver.quit();
		System.out.println("Done with Claim IT Report- with Provider \n\n");
		if (fis != null)
				fis.close();
