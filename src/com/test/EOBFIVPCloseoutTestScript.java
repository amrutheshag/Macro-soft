...
public class EOBFIVPCloseoutTestScript {
	WebDriver driver;
	FileInputStream fis;
		@Test
		public void EOBFIVPCloseoutExtractClaimsDataReportTestScript() throws Exception{
			DOMConfigurator.configure("log4j.xml");
			System.out.println("\n*****************************************************\n");
			Log.info("\n*****************************************************\n");
			System.out.println("Executing EOB FIVP Closeout extract claims data report");
			Log.info("Executing EOB FIVP Closeout extract claims data report");
			
			
			try{
				String Path=Constant.Path_TestData + Constant.File_Edit_EOBQueryData;
			   	int rowCount=ExcelUtils.setExcelFile(Path, "FIVP");
		    	for (int count=1; count<=rowCount; count++){
		    		...
		    		System.out.println("\n----------------------------------------------------\n");
		   		 	System.out.println("EOB FIVP CLoseout Extract claims data report - Running Iteration No:  " + ExcelUtils.getCellData(count,0));
		   		 	System.out.println("\n----------------------------------------------------\n");
		   		 	Log.info("----------------------------------------------------");
		   		 	Log.info("EOB  FIVP Closeout Extract claims data report- Running Iteration No:  " + ExcelUtils.getCellData(count,0));
		   		 	Log.info("----------------------------------------------------");
		...
		System.out.println("Done with EOB FIVP Closeout Extract claims data report \n\n");
		if (fis != null)
				fis.close();
	}
}
