...
public class ClaimITReportSearchOnClaimQuery {
	WebDriver driver;
	FileInputStream fis;
		@Test
		public void ClaimITReportWithClaimQueryTestScript() throws Exception{
			DOMConfigurator.configure("log4j.xml");
			System.out.println("\n*****************************************************\n");
			Log.info("\n*****************************************************\n");
			System.out.println("Executing Claim IT Report with Claim Query");
			Log.info("Executing Claim IT Report with Claim Query");
			...
			try{
		    	...
		    	for (int count=1; count<=rowCount; count++){
		    		...
		    		System.out.println("\n----------------------------------------------------\n");
		   		 	System.out.println("Claim IT Report with claim query- Running Iteration No:  " + ExcelUtils.getCellData(count,0));
		   		 	System.out.println("\n----------------------------------------------------\n");
		   		 	Log.info("----------------------------------------------------");
		   		 	Log.info("Claim IT Report with claim query- Running Iteration No:  " + ExcelUtils.getCellData(count,0));
		   		 	Log.info("----------------------------------------------------");
		   		 claimITReportWithClaimQueryMultiple(driver, approvedServiceOnly, openClaimsOnly, extractClaimAndServiceData, denyServices, providerID, memberID, payor, from, to, claimQuery, insurerPayor, checkBilaterals, checkCCI, checkGlobalPeriods, checkLCPDiagnosis, assistantSurgeon);
		    		}
		    	}
			...
			//print claim IT Report with claim query
			ClaimITReportPage ClaimITReportPageObject = new ClaimITReportPage(driver);
			ClaimITReportPageObject.searchOnClaimQueryPrintClaimITReport( approvedServiceOnly, openClaimsOnly, extractClaimAndServiceData, denyServices, providerID, memberID, payor, from, to, claimQuery, insurerPayor, checkBilaterals, checkCCI, checkGlobalPeriods, checkLCPDiagnosis, assistantSurgeon);
			}

		@AfterMethod
		public void afterMethod() throws Exception {
		if (driver!=null) driver.quit();
		System.out.println("Done with Claim IT Report- with Claim Query \n\n");
		if (fis != null)
				fis.close();
	}
}
