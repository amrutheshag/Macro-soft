...
public class LateClaimInterestRateExit {
	WebDriver driver;
	FileInputStream fis;
		@Test
		public void LateClaimsInterestRatesExitTestScript() throws Exception{
			DOMConfigurator.configure("log4j.xml");
			System.out.println("\n*****************************************************\n");
			Log.info("\n*****************************************************\n");
			System.out.println("Executing Late Claims Interest Rates- Exit");
			Log.info("Executing  Late Claims Interest Rates Exit ");
			...
			try{
		    	...
		    	for (int count=1; count<=rowCount; count++){
		    		...
		    		System.out.println("\n----------------------------------------------------\n");
		   		 	System.out.println("Late Claims Interest Rates Exit- Running Iteration No:  " + ExcelUtils.getCellData(count,0));
		   		 	System.out.println("\n----------------------------------------------------\n");
		   		 	Log.info("----------------------------------------------------");
		   		 	Log.info("Late Claims Interest Rates Exit- Running Iteration No:  " + ExcelUtils.getCellData(count,0));
		   		 	Log.info("----------------------------------------------------");
   		 	exitInterestRatesMultiple(driver,effectiveDate, state, benefitProduct);
		    		}
		    	}
			...
			public void exitInterestRatesMultiple(WebDriver driver,String effectiveDate, String state, String benefitProduct) throws Exception
			{
			...
			claimInterestObject.InterestRateExit( effectiveDate, state, benefitProduct);
		...
		System.out.println("Done with Late Claim Interest Rates-Exit  \n\n");
		if (fis != null) 
				fis.close();
	}
}
